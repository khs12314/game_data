...
namespace game_data
{
    class PenaltyKickGame
    {
        ...
        static bool IsNineDigitNumber(string input)
        {
            // 문자열이 null이 아니고, 길이가 9이며, 모두 숫자인지 확인
            if (input != null && input.Length == 9 && int.TryParse(input, out _))
            {
                return true;
            }
            return false;
        ...
        static string IDinput()
        {
            ...
            Console.WriteLine("9자리의 학번을 입력해주세요.");
            IDNum = Console.ReadLine();
            while (!IsNineDigitNumber(IDNum))
            {
                
                Console.WriteLine("잘못된 입력입니다. 9자리의 학번을 입력해주세요.");
                IDNum = Console.ReadLine();
            }
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                using (StreamReader reader = new StreamReader("game_data"+"1"+".txt"))
                {
            ...
            while ((totalSuccess < 4)&&(count<5))
            {
                ...
                {
                    ggst(idn, input, totalSuccess, count, guid);
        ...
        static void ggst(string id, string input,byte total, byte cnt, Guid? guid)
        {
            GoogleCredential credential;
            // 사용자 인증 정보 로드
            using (var stream = new FileStream(@"gamelog-420512-010a0df65b4a.json", FileMode.Open, FileAccess.Read))
            ...
            ValueRange valueRange = new ValueRange();
            valueRange.Values = new List<IList<object>> {
            new List<object> {"Time",DateTime.Now.ToString("G"), "", "학번", id, "킥 or 도핑", input,"방향","","골키퍼의 방향값","","해당 회차 성공횟수",total,"해당 회차 시도 횟수",cnt,"플레이 시드", guid}
        };
        ...
        static void addplaytime(SheetsService service, string spreadsheetId, string sheetName, string id, int a)
        {
            ...
            if (values != null && values.Count > 0)
            {
                foreach (var row in values)
                {
                    ...
                    oldtime = int.Parse(row[1].ToString());
                    if ((InSpId == id)&&(maxtime<oldtime))
                    {
                        maxtime = oldtime;
                    }
